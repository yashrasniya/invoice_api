...
from upi_qr import company_upi_link, make_qr_image
...
class Point:
    ...
    def add_suffix(self):
        # An affix on an empty field used to render as "UPI : None" — decorate
        # only values that are actually there.
        if self.suffix is not None and not self._is_blank():
    ...
    def add_prefix(self):
        if self.prefix and not self._is_blank():
...
class Polygon:
    def __init__(self, title, points=None, **kwargs):
        ...
        # Generated artwork (the UPI QR) that has no URL to fetch — the
        # renderer prefers this over `src` when it is set.
        self.src_image = None
...
class FillValue:
    ...
    def invoice_total(self):
        """Grand total of this invoice, as the footer computed it."""
        total = self.raw_footer_data.get("total_amount_with_gst")
        if total is None:
            total = self.data.get("total_final_amount")
        return total
    ...
    def set_my_company_data(self,request):
        if request.user.user_company and request.user.user_company.is_varified:
            ...
            # Amount-bearing QR: encodes this invoice's total, so it is built
            # per invoice rather than stored on the company like the logo is.
            # None here means "this company prints no QR" and blanks both the
            # code and its `upi_id` caption, so an opted-out invoice never
            # shows half a payment block.
            upi_link = company_upi_link(
                obj,
                self.invoice_total(),
                note=self.data.get('invoice_number') or None,
            )
            upi_qr_image = make_qr_image(upi_link)
            for i in self.my_company_details + self.footers:
                if getattr(i, 'rectangles_type', None) == 'image' and i.label == "upi_qr":
                    i.src_image = upi_qr_image
                    continue
                if i.type() == 'Point' and str(i.label).lower() == 'upi_id':
                    i.value = obj.upi_id if upi_link else ''
                    continue
